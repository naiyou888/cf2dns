...
TCP_NOTSENT_LOWAT_MODE="${TCP_NOTSENT_LOWAT_MODE:-auto}"
TCP_NOTSENT_LOWAT_CUSTOM=""
IEPL_CAP_MB="${IEPL_CAP_MB:-16}"
...
Options:
  ...
  --tcp-notsent-lowat auto|off|N    Default: auto. landing=off, iepl=off.
                                    Use N bytes only when you deliberately want application write throttling.
  --iepl-cap-mb 8|16|32             IEPL role max TCP buffer cap in MiB. Default: 16.
                                    8 = lower latency, 16 = balanced, 32 = more aggressive.
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --tcp-notsent-lowat)
      case "${2:-}" in
        auto|off) TCP_NOTSENT_LOWAT_MODE="${2:-}"; TCP_NOTSENT_LOWAT_CUSTOM="" ;;
        *)
          if [[ "${2:-}" =~ ^[0-9]+$ ]]; then
            TCP_NOTSENT_LOWAT_MODE="custom"; TCP_NOTSENT_LOWAT_CUSTOM="${2:-}"
          else
            echo "ERROR: --tcp-notsent-lowat must be auto, off, or integer bytes." >&2; exit 1
          fi
          ;;
      esac
      shift 2 ;;
    --iepl-cap-mb) IEPL_CAP_MB="${2:-}"; shift 2 ;;
...
if [[ ! "$TCP_NOTSENT_LOWAT_MODE" =~ ^(auto|off|custom)$ ]]; then
  echo "ERROR: --tcp-notsent-lowat must be auto, off, or integer bytes." >&2
  exit 1
fi
if [[ ! "$IEPL_CAP_MB" =~ ^(8|16|32)$ ]]; then
  echo "ERROR: --iepl-cap-mb must be 8, 16, or 32." >&2
  exit 1
fi
...
if [[ "$ROLE" == "iepl" ]]; then
  ...
  PROFILE_BUF_CAP=$((IEPL_CAP_MB * MiB))
  TCP_BUF_MIN=$((8 * MiB))
  SOMAX=8192
  SYN_BACKLOG=8192
  NETDEV_BACKLOG=8192
...
# tcp_notsent_lowat policy: keep it off by default for AnyTLS/sing-box.
# It can reduce unsent data queueing in some cases, but may throttle proxy long-connection throughput.
case "$TCP_NOTSENT_LOWAT_MODE" in
  off|auto)
    SET_TCP_NOTSENT_LOWAT=0
    TCP_NOTSENT_LOWAT_VALUE=0
    ;;
  custom)
    SET_TCP_NOTSENT_LOWAT=1
    TCP_NOTSENT_LOWAT_VALUE="$TCP_NOTSENT_LOWAT_CUSTOM"
    ;;
esac
...
print_report() {
  ...
  echo "tcp_notsent_lowat:    ${TCP_NOTSENT_LOWAT_MODE}${TCP_NOTSENT_LOWAT_CUSTOM:+ (${TCP_NOTSENT_LOWAT_CUSTOM})}"
  [[ "$ROLE" == "iepl" ]] && echo "IEPL buffer cap:      ${IEPL_CAP_MB} MiB"
